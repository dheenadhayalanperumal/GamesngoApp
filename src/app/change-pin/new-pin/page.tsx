...
export default function NewPinPage() {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    if (e.key === 'Backspace' && !(isConfirming ? confirmPin[index] : pin[index]) && index > 0) {
      setCurrentIndex(index - 1);
    ...
    <div style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', margin: '0 -15px' }}>
     ...
     />
      ...
      <Box sx={{ 
        ...
        >
          {isConfirming 
            ? 'Please confirm your new PIN to complete the setup.'
          ...
          {[0, 1, 2, 3].map((index) => (
            <Box
              ...
              sx={{
                ...
                height: '60px',
                borderRadius: '50%',
                backgroundColor: getCurrentPin()[index] ? '#4848DB' : '#f5f5f5',
                border: getCurrentPin()[index] ? 'none' : '2px solid #e0e0e0',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                position: 'relative',
